...
function generateUniqueId() {
  // Menggabungkan waktu saat ini (ms) dan angka acak.
...
const todoText = prompt("Masukkan teks to-do baru: ");

// 1. Minta input dan 2. Validasi input
if (!todoText || todoText.trim() === "") {
  console.log("❌ Teks to-do tidak boleh kosong. Pembatalan penambahan.");
  return;
}
  // 3. Buat objek to-do baru dengan properti: id (dari generateUniqueId), text, dan isCompleted (boolean, default false)
  const newTodo = {
    id: generateUniqueId(),
    text: todoText.trim(), // Pastikan tidak ada spasi berlebihan
    isCompleted: false,
  };
...
function markTodoCompleted() {
  if (todos.length === 0) {
    console.log("ℹ️ Tidak ada to-do. Tambahkan to-do terlebih dahulu.");
    return;
  }
 ...
 listTodos();
  ...
  const input = prompt(
    "Masukkan nomor to-do yang ingin ditandai selesai (atau 'batal'): "
  );

  if (input.toLowerCase() === "batal") {
    console.log("↪️ Aksi dibatalkan.");
    return;
  }

  const indexToMark = parseInt(input) - 1; // Konversi nomor urut ke indeks array
  // 3. Validasi input: Pastikan nomor adalah angka, dalam rentang yang valid (1 sampai jumlah to-do)
   if (isNaN(indexToMark) || indexToMark < 0 || indexToMark >= todos.length) {
    console.log("❌ Nomor to-do tidak valid. Pembatalan aksi.");
    return;
  }

  const todoToMark = todos[indexToMark];
  // 6. Tangani kasus jika to-do sudah selesai
  if (todoToMark.isCompleted) {
    console.log(
      `ℹ️ To-do #${indexToMark + 1} ("${todoToMark.text}") sudah selesai.`
    );
    return;
  }
 ...
 todoToMark.isCompleted = true;
  ...
  // 5. Beri feedback ke user bahwa to-do berhasil ditandai selesai
   console.log(
    `🎉 To-do #${indexToMark + 1} berhasil ditandai sebagai selesai!`
  );
}

function deleteTodo() {
  if (todos.length === 0) {
    console.log("ℹ️ Tidak ada to-do. Tidak ada yang bisa dihapus.");
    return;
  }
  ...
  const input = prompt(
    "Masukkan nomor to-do yang ingin dihapus (atau 'batal'): "
  );
  ...
  const indexToDelete = parseInt(input) - 1; // Konversi nomor urut ke indeks array
 ...
 const deletedTodo = todos.splice(indexToDelete, 1)[0];
  ...
  console.log(`🗑️ To-do berhasil dihapus: "${deletedTodo.text}"`);
}
...
function listTodos() {
  ...
  // 2. Cek apakah array `todos` kosong. Jika ya, tampilkan pesan "No to-dos to display."
   if (todos.length === 0) {
     console.log("🥳 Tidak ada to-do untuk ditampilkan. Saatnya bersantai!");
   } else {
...
todos.forEach((todo, index) => {
     ...
     console.log(`${displayIndex}. ${status} | ${todo.text}`);
    });
...
function runTodoApp() {
  let running = true;
  console.log("🚀 Aplikasi To-Do List Berbasis Konsol Dimulai!");
  ...
  console.log("\n--- MENU ---");
    console.log(
      "➡️ Perintah: **add** | **list** | **complete** | **delete** | **exit**"
    );
    // 2. Minta user memasukkan perintah menggunakan `prompt()`
     const command = prompt("Masukkan perintah: ").toLowerCase().trim();
    ...
    // 4. Tangani perintah 'exit' untuk menghentikan loop aplikasi
        console.log("👋 Sampai jumpa! Aplikasi To-Do List dihentikan.");
        running = false;
        break;
      default:
    // 5. Tangani input perintah yang tidak valid
          console.log(
          `🚫 Perintah tidak dikenal: '${command}'. Silakan coba lagi.`
        );
        break;
    }
  }
